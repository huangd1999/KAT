...
class WideResNet(nn.Module):
    ...
    def forward(self, x, epoch=60):
        cifar10_mean = torch.tensor([0.4914, 0.4822, 0.4465]).cuda()# equals np.mean(train_set.train_data, axis=(0,1,2))/255
        cifar10_std = torch.tensor([0.2471, 0.2435, 0.2616]).cuda() 
        x = (x - cifar10_mean.view(1,3,1,1)) / cifar10_std.view(1,3,1,1)
        out = self.conv1(x)
...
class WideResNet_KAT(nn.Module):
    def __init__(self, depth=34, num_classes=10, widen_factor=10, dropRate=0.0):
        super(WideResNet_KAT, self).__init__()
        nChannels = [16, 16 * widen_factor, 32 * widen_factor, 64 * widen_factor]
        assert ((depth - 4) % 6 == 0)
        n = (depth - 4) / 6
        block = BasicBlock
        ...
        self.bn1 = nn.BatchNorm2d(nChannels[3])
        self.relu = nn.ReLU(inplace=True)
        self.fc = KAN([nChannels[3], num_classes])
        self.nChannels = nChannels[3]
...
def WideResNet(KAT=False,num_classes=10):
    if KAT:
       return WideResNet_KAT(num_classes=10)
    else:
