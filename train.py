    ...
    for epoch in range(1,args.epoch+1):
        ...
        if epoch%10==0 or epoch > int(0.7 * args.epoch):
            ...
            if acc_ori + acc_adv > best_acc:
                ...
                if args.kat:
                    saved_name = '{0}-{1}-kat-{2}.pt'.format(args.dataset, args.model,args.loss)
                else:
                    saved_name = '{0}-{1}-{2}.pt'.format(args.dataset, args.model,args.loss)
                torch.save(model, os.path.join(args.save_path, saved_name))
            if acc_adv > best_robustness:
                best_robustness = acc_adv
                if args.kat:
                    saved_name = 'best-{0}-{1}-kat-{2}.pt'.format(args.dataset, args.model,args.loss)
                else:
                    saved_name = 'best-{0}-{1}-{2}.pt'.format(args.dataset, args.model,args.loss)
                torch.save(model,os.path.join(args.save_path, saved_name))
...
if __name__ =='__main__':
    ...
    parser.add_argument('--kat', type=bool, default=False, help='KAN | MLP') 
    args = parser.parse_args()


    logfile = os.path.join(args.out_dir, '{0}-{1}-{2}-{3}.log'.format(args.model,args.dataset,args.loss,args.kat))
    ...
    if args.model == 'resnet18':
        model = ResNet18(args.kat,num_classes).cuda()
    else:
        model = WideResNet(args.kat,num_classes).cuda()
